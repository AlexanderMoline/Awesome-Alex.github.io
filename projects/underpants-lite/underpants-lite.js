...
/*_.contains = function(array, value) {
  var contain = _.indexOf(array, value);
  for (let i = 0; i <= array.length; i++) {
   if (index === -1) {
    return false;
   }
...
_.map = function(collection, fn) {
 ...
 if (Array.isArray(collection)) {
  for (let i = 0; i < collection.length; i++) {
   array.push(fn(collection[i], i, collection));
  ...
  for (let key in collection) {
   array.push(fn(collection[key], key, collection));
  }
...
_.partition = function(array, fn) {
 ...
 _.filter(array, function(values, loc, collection) {
  if (fn(values, loc, collection)) {
...
_.pluck = function(array, property) {
 const arr = _.map(array, function(elt, i, array) {
  return elt[property];
 });
